...
export class TodosComponent implements OnInit {
  ...
  updateData(formData, id) {
    this.itemService.updateItem(formData, id).subscribe(
      (response) => {
        this.items[this.branchIndex] = response;
        this.closeModal();
  ...
  editTodo(index: any) {
    ...
    const branch = this.items[index];
    for ( const key in branch ) {
        if ( key === 'areas' ) {
          this.serAreas( branch[key] );
        } else if ( this.branchForm.get( key ) ) {
            this.branchForm.get(key).setValue( branch[key] );
        }
    }
    this.showModal();
  }
